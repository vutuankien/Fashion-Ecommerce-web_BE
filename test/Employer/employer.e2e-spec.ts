...
/** Mô tả bộ test cho EmployerController (Các trường hợp lỗi - Unhappy Cases) */
describe('EmployerController (e2e) - Unhappy Cases', () => {
    ...
    describe('Trường hợp dữ liệu không hợp lệ (400 Bad Request)', () => {
        ...
        it('POST /employer - Phải trả về 400 khi thiếu trường bắt buộc', () => {
            /** Thực hiện request thiếu dữ liệu, có kèm token admin */
    ...
    describe('Kiểm tra Authentication & Authorization (401 Unauthorized)', () => {
        it('POST /employer - Phải trả về 401 khi không có token', () => {
            return request(app_instance.getHttpServer())
                .post('/employer')
                .send({})
                .expect(401);
        ...
        it('GET /employer - Phải trả về 401 khi không có token', () => {
            return request(app_instance.getHttpServer())
                .get('/employer')
                .expect(401);
        });

        it('GET /employer/:id - Phải trả về 401 khi không có token', () => {
            return request(app_instance.getHttpServer())
                .get('/employer/123')
                .expect(401);
        });

        it('PATCH /employer/:id - Phải trả về 401 khi không có token', () => {
            return request(app_instance.getHttpServer())
                .patch('/employer/123')
                .send({})
                .expect(401);
        ...
        it('DELETE /employer/:id - Phải trả về 401 khi không có token', () => {
            return request(app_instance.getHttpServer())
                .delete('/employer/123')
                .expect(401);
        });
    ...
    describe('Trường hợp không tìm thấy dữ liệu (404 Not Found)', () => {
        /** Test case lấy thông tin employer với ID không có trong database */
        it('GET /employer/:id - Phải trả về lỗi khi ID không tồn tại', () => {
            /** Gửi request với ID giả định */
            return request(app_instance.getHttpServer())
                .get('/employer/nonexistent-employer-id-999')
                .set('Authorization', `Bearer ${admin_token}`)
                /** Kỳ vọng mã trạng thái 404 do service ném lỗi */
                .expect(404);
        });

        /** Test case cập nhật employer không tồn tại */
        it('PATCH /employer/:id - Phải trả về lỗi khi cập nhật employer không tồn tại', () => {
            /** Gửi request cập nhật với ID không tồn tại */
            return request(app_instance.getHttpServer())
                .patch('/employer/nonexistent-employer-id-999')
                .set('Authorization', `Bearer ${admin_token}`)
                .send({
        ...
        /** Test case xóa employer không tồn tại */
        it('DELETE /employer/:id - Phải trả về lỗi khi xóa employer không tồn tại', () => {
            /** Gửi request xóa với ID không tồn tại */
            return request(app_instance.getHttpServer())
                .delete('/employer/nonexistent-employer-id-999')
                .set('Authorization', `Bearer ${admin_token}`)
                /** Kỳ vọng mã trạng thái 404 */
                .expect(404);
        });
