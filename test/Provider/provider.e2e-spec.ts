...
/** Mô tả bộ test cho ProviderController (Các trường hợp lỗi - Unhappy Cases) */
describe('ProviderController (e2e) - Unhappy Cases', () => {
    ...
    describe('Trường hợp dữ liệu không hợp lệ (400 Bad Request)', () => {
        ...
        it('POST /provider - Phải trả về 400 khi thiếu trường bắt buộc', () => {
    ...
    describe('Trường hợp không tìm thấy dữ liệu (404 Not Found hoặc 500)', () => {
        /** Test case lấy thông tin provider với ID không có trong database */
        it('GET /provider/:id - Phải trả về lỗi khi ID không tồn tại', () => {
            /** Gửi request với ID giả định */
            return request(app_instance.getHttpServer())
                .get('/provider/nonexistent-provider-id-999')
                /** Kỳ vọng mã trạng thái 500 do service ném lỗi */
                .expect(404);

        });

        /** Test case cập nhật provider không tồn tại */
        it('PATCH /provider/:id - Phải trả về lỗi khi cập nhật provider không tồn tại', () => {
            /** Gửi request cập nhật với ID không tồn tại */
            return request(app_instance.getHttpServer())
                .patch('/provider/nonexistent-provider-id-999')
                .send({
        ...
        /** Test case xóa provider không tồn tại */
        it('DELETE /provider/:id - Phải trả về lỗi khi xóa provider không tồn tại', () => {
            /** Gửi request xóa với ID không tồn tại */
            return request(app_instance.getHttpServer())
                .delete('/provider/nonexistent-provider-id-999')
                /** Kỳ vọng mã trạng thái 500 */
                .expect(404);
        });
    });
