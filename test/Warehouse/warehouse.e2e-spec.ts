...
/** Mô tả bộ test cho WarehouseController (Các trường hợp lỗi - Unhappy Cases) */
describe('WarehouseController (e2e) - Unhappy Cases', () => {
    ...
    describe('Trường hợp dữ liệu không hợp lệ (400 Bad Request)', () => {
        ...
        it('POST /warehouse - Phải trả về 400 khi thiếu trường bắt buộc', () => {
    ...
    describe('Trường hợp không tìm thấy dữ liệu (404 Not Found hoặc 500)', () => {
        /** Test case lấy thông tin warehouse với ID không có trong database */
        it('GET /warehouse/:id - Phải trả về lỗi khi ID không tồn tại', () => {
            /** Gửi request với ID giả định */
            return request(app_instance.getHttpServer())
                .get('/warehouse/nonexistent-warehouse-id-999')
                /** Kỳ vọng mã trạng thái 500 do service ném lỗi */
                .expect(404);
        });

        /** Test case cập nhật warehouse không tồn tại */
        it('PATCH /warehouse/:id - Phải trả về lỗi khi cập nhật warehouse không tồn tại', () => {
            /** Gửi request cập nhật với ID không tồn tại */
            return request(app_instance.getHttpServer())
                .patch('/warehouse/nonexistent-warehouse-id-999')
                .send({
        ...
        /** Test case xóa warehouse không tồn tại */
        it('DELETE /warehouse/:id - Phải trả về lỗi khi xóa warehouse không tồn tại', () => {
            /** Gửi request xóa với ID không tồn tại */
            return request(app_instance.getHttpServer())
                .delete('/warehouse/nonexistent-warehouse-id-999')
                /** Kỳ vọng mã trạng thái 500 */
                .expect(404);
        });
    });
