...
/** Mô tả bộ test cho CategoriesController (Các trường hợp lỗi - Unhappy Cases) */
describe('CategoriesController (e2e) - Unhappy Cases', () => {
    ...
    describe('Trường hợp dữ liệu không hợp lệ (400 Bad Request)', () => {
        ...
        it('POST /categories - Phải trả về 400 khi thiếu trường bắt buộc', () => {
    ...
    describe('Trường hợp không tìm thấy dữ liệu (404 Not Found hoặc 500)', () => {
        /** Test case lấy thông tin category với ID không có trong database */
        it('GET /categories/:id - Phải trả về lỗi khi ID không tồn tại', () => {
            /** Gửi request với ID giả định */
            return request(app_instance.getHttpServer())
                .get('/categories/nonexistent-category-id-999')
                /** Kỳ vọng mã trạng thái 500 do service ném lỗi */
                .expect(404);
        });

        /** Test case cập nhật category không tồn tại */
        it('PATCH /categories/:id - Phải trả về lỗi khi cập nhật category không tồn tại', () => {
            /** Gửi request cập nhật với ID không tồn tại */
            return request(app_instance.getHttpServer())
                .patch('/categories/nonexistent-category-id-999')
                .send({
        ...
        /** Test case xóa category không tồn tại */
        it('DELETE /categories/:id - Phải trả về lỗi khi xóa category không tồn tại', () => {
            /** Gửi request xóa với ID không tồn tại */
            return request(app_instance.getHttpServer())
                .delete('/categories/nonexistent-category-id-999')
                /** Kỳ vọng mã trạng thái 404 */
                .expect(404);
        });
    });
